...
import { getBlogPosts } from "@/app/actions/adminActions"; // Import Server Action
...
export default function BlogPage() {
  ...
  // --- Logic: Separate Featured Post & Filter ---
  // Find the first post marked as featured, or default to the most recent one
  const featuredPost = posts.find(post => post.featured) || posts[0];
  
  // Exclude the featured post from the main grid
  const regularPosts = posts.filter(post => post.id !== featuredPost?.id);

  // Apply filters to regular posts
          ...
          <>
            ...
            {featuredPost && (
              <motion.div 
                ...
                className="bg-white rounded-[2rem] p-6 md:p-8 shadow-2xl shadow-slate-900/10 border border-slate-100 mb-16 flex flex-col lg:flex-row gap-8 items-center"
              >
                {/* Image */}
                ...
                <div className="w-full lg:w-1/2 flex flex-col justify-center">
                   ...
                   <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-4 leading-tight group-hover:text-teal-600 transition-colors cursor-pointer">
                      {featuredPost.title}
                   </h2>
                   ...
                   <Link href={`/blog/${featuredPost.slug}`}>
                     <button className="group inline-flex items-center gap-2 text-teal-600 font-bold text-lg hover:text-teal-800 transition-colors">
                        Read Article
                    ...
                    >
                      ...
                      <div className="p-6 flex flex-col flex-grow">
                         ...
                         <Link href={`/blog/${post.slug}`} className="mt-auto">
                           <span className="inline-flex items-center gap-2 text-sm font-bold text-slate-900 group-hover:text-teal-600 transition-colors">
        ...
        <div className="mt-20 bg-slate-900 rounded-[2.5rem] p-8 md:p-16 text-center relative overflow-hidden">
           ...
           <div className="relative z-10 max-w-2xl mx-auto">
              ...
              <form className="flex flex-col sm:flex-row gap-4">
                 ...
                 />
                 <button className="px-8 py-4 bg-teal-600 hover:bg-teal-500 text-white font-bold rounded-xl transition-all shadow-lg hover:shadow-teal-500/20">
                    Subscribe
                 </button>
              </form>
